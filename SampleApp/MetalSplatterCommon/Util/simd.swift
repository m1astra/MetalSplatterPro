...
public extension simd_quatf
{
    ...
    func yawPitchRoll() -> SIMD3<Float> {
        let m = float4x4(self)

        let yaw = atan2(m.columns.0.z, m.columns.2.z)
        let pitch = asin(-m.columns.1.z)
        let roll = atan2(m.columns.1.x, m.columns.1.y)

        return SIMD3(yaw, pitch, roll)
    }
    
    func flipYaw() -> simd_quatf {
        let ypr = self.yawPitchRoll()

        let flippedYaw = -ypr.x

        return simd_quatf(
            angle: flippedYaw, axis: [0, 1, 0]
        )
        * simd_quatf(
            angle: ypr.y, axis: [1, 0, 0]
        )
        * simd_quatf(
            angle: ypr.z, axis: [0, 0, 1]
        )
    }
}
...
public func simd_look(at: SIMD3<Float>, up: SIMD3<Float> = SIMD3<Float>(0, 1, 0)) -> simd_float4x4 {
